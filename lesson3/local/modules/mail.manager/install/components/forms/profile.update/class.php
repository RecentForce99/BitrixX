...
class EmailManagerComponent extends CBitrixComponent
{
    ...
    public function executeComponent()
    {
        ...
        $this->emails = $profile->getConnection();
        $this->idIBlock = '';
        ...
        if ($this->templateName == 'grid') {
            ...
            $this->arResult = $profile->getConnection();
        }
    ...
    private function getGridBody(): array
    {
        ...
        foreach ($this->emails['PROFILES'] as $arItem) {
            ...
            $arGridElement['actions'] = [
                [
                    'text' => Loc::getMessage('RF.CARD.LIST.CLASS.EDIT'), //Доделать
                    'onclick' => 'document.location.href="/"'
                ],
                [
                    'text' => 'Удалить', //Доделать,
                    'onclick' => Loc::getMessage('RF.CARD.LIST.CLASS.DELETE')
                ]
            ];
    ...
    {
        return 'ylab_email_manager_' . $this->idIBlock;
    }
